  ...
  try {
    ...
    switch (response.statusCode) {
      case 200:
        ...
        if (kDebugMode) {
          debugPrint('API call successful: ${response.data['data']['token']}');
        }

        const storage = FlutterSecureStorage();
        await storage.write(
            key: 'access_token', value: await response.data['data']['token']);
        await storage.write(
            key: 'refresh_token',
            value: await response.data['data']['refreshToken']);
